...
ifneq (,$(or USE_ST_CMSIS, USE_ST_HAL))
    ...
    CPPFLAGS += -I ./Drivers/CMSIS/ARM/inc
    CPPFLAGS += -I ./Drives/CMSIS/$(SERIES_FOLDER)/inc

    SRC += ./Drivers/CMSIS/$(SERIES_FOLDER)/src/*.c
endif

# Include the HAL files
ifdef USE_ST_HAL
    CPPFLAGS += -D USE_HAL_DRIVER
    CPPFLAGS += -I ./Drivers/HAL/$(SERIES_FOLDER)/inc

    # A simply expanded variable is used here to perform the find command only once.
    HAL_SRC := $(shell find ./Drivers/HAL/$(SERIES_FOLDER)/src/*.c ! -name '*_template.c')
    SRC += $(HAL_SRC)
endif
